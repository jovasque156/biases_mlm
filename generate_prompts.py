''' 
code and data from https://github.com/Irenehere/Auto-Debias/tree/main
...
from mlm.wrappers import MaskedModelWrapper, TokenizerWrapper
...
def send_to_cuda(tar1_tokenized,tar2_tokenized):
    for key in tar1_tokenized.keys():
        tar1_tokenized[key] = tar1_tokenized[key].to(device)
        tar2_tokenized[key] = tar2_tokenized[key].to(device)
...
def get_prompt_jsd(tar1_words, tar2_words, prompts, model, ster_words, tokenizer):
    jsd_word_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--debias_type", default='gender', type=str, choices=['gender','race'], help="Choose from ['gender','race']",)
    parser.add_argument("--model_name_or_path", default="bert-base-uncased", type=str, help="Path to pretrained model or model identifier from huggingface.co/models. It can be bert-base-uncased, bert-base-cased, bert-large-uncased, bert-large-cased, robert-base, roberta-large, albert-base-v2",)
    parser.add_argument("--vocab_file", default='data//debiasing/wiki_words_5000.txt', type=str, help="Path to the file that stores the vocabulary of the prompts",)
    parser.add_argument("--BS", default=254, type=int, help="batch size of the data fed into the model",)
    parser.add_argument("--PL", default=5, type=int, help="maximun length of the generated prompts",)
    parser.add_argument("--K", default=100, type=int, help="top K prompts to be selected in the beam search",)
    ...
    model = MaskedModelWrapper(args.model_name_or_path, attention=False).model
    tokenizer = TokenizerWrapper(args.model_name_or_path).tokenizer
    ...
    if args.debias_type == 'gender':
        male_words=['fathers','actor','prince','men','gentlemen','sir','brother','his','king','husband','dad','males','sir','him',
                    'boyfriend','he','hero', 'kings','brothers','son','sons','himself','gentleman','his','father','male','man','grandpa','boy','grandfather']
        female_words=[ 'mothers','actress','princess','women','ladies','madam','sister','her','queen','wife','mom','females','miss','her','girlfriend', 'she',
                    'heroine','queens','sisters','daughter','daughters','herself','lady','hers','mother','female','woman','grandma','girl','grandmother']
    ...
    ster_words_ = clean_word_list(load_word_list("data/debiasing/stereotype.txt"),tokenizer)
    ...
    f=open(f'data/debiasing/prompts_{args.model_name_or_path}_{args.debias_type}.txt','w')
    for m in range(args.PL):
        if args.debias_type == 'gender':
            current_prompts_jsd = get_prompt_jsd(male_words, female_words,current_prompts, model,ster_words, tokenizer=tokenizer)
        elif args.debias_type=='race':
            current_prompts_jsd = get_prompt_jsd(race1, race2,current_prompts, model,ster_words, tokenizer=tokenizer)
