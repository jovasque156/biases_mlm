...
if __name__ == "__main__":
    ...
    # parser.add_argument("--model_type", default="bert", type=str, help="choose from ['bert','roberta','albert']",)
    parser.add_argument("--data_path", default="data/debiasing/", type=str, help="data path to put the taget/attribute word list",)
    parser.add_argument("--prompts_file", default="", type=str, help="the name of the file that stores the prompts, by default it is under the data_path",)
    parser.add_argument("--batch_size", default=1, type=int, help="batch size in auto-debias fine-tuning",)
    parser.add_argument("--lr", default=5e-6, type=float, help="learning rate in auto-debias fine-tuning",)
    parser.add_argument("--epochs", default=1, type=int, help="number of epochs in auto-debias fine-tuning",)
    parser.add_argument("--finetuning_vocab_file", default=None, type=str, help="vocabulary to be fine-tuned in auto-debias fine-tuning, if None, tune the whole vocabulary.",)
    parser.add_argument("--tune_pooling_layer", default=False,type=str,help="whether to tune the pooling layer with the auxiliary loss",)
    ...
    for i in range(1,args.epochs+1):  
        ...
        for idx in idx_ds:
            ...
            if args.finetuning_vocab_file:   
                tar1_predictions_logits = tar1_predictions.logits[torch.arange(tar1_predictions.logits.size(0)), tar1_mask][:, finetuning_vocab]
                tar2_predictions_logits = tar2_predictions.logits[torch.arange(tar2_predictions.logits.size(0)), tar2_mask][:, finetuning_vocab]
            else:
            ...
            tar1_mask=None
            tar2_mask=None
            tar1_predictions=None
            loss = None
            embed_dist = None
            jsd_loss = None
            tar1_predictions_logits = None
            tar2_predictions_logits = None
                ...
                torch.save(
                    {
                        ...
                        f'results/{args.model_name_or_path}/debiased_model_{args.debias_type}_using_torch.pth'
                )
                model.save_pretrained(f'results/{args.model_name_or_path}/debiased_model_{args.debias_type}')
                tokenizer.save_pretrained(f'results/{args.model_name_or_path}/debiased_model_{args.debias_type}')
